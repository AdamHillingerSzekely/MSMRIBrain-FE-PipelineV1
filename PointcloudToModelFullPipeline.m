...
targetFaces=13244;
%min 1554
%targetFaces=round(numFaces/20);
%disp(['Number of faces in the mesh brain: ', num2str(targetFaces)]);

[Ft_simplified, Vt_simplified] = reducepatch(Ft, Vt, targetFaces);
Vt= Vt_simplified;
Ft= Ft_simplified;
...
for clusterW = 1:numClustersW
    ...
    numFacesW = size(FtW, 1);
    disp(numFacesW)
    

    targetFacesW=7209;
    %min faces 604
    %targetFacesW=round(numFacesW/10);




    % Count the number of faces
    [FtW_simplified, VtW_simplified] = reducepatch(FtW, VtW, targetFacesW);
    VtW= VtW_simplified;
    FtW= FtW_simplified;
    ...
    disp(['Number of faces in the WM mesh: ', num2str(numFacesW)]);
...
for clusterL = 1:numClustersL
    ...
    [FtL,VtL]=mergeVertices(FtL,VtL);
    
    
    numFacesL = size(FtL, 1);
    ...
    %targetFaces=500;
    %min faces 6/7
    targetFacesL=round(25*numFacesL/28);
    disp(targetFacesL);
    



    [FtL_simplified, VtL_simplified] = reducepatch(FtL, VtL, targetFacesL);
    VtL= VtL_simplified;
    FtL= FtL_simplified;
    ...
    disp(['Number of faces in the Lesion mesh: ', num2str(numFacesL)]);
